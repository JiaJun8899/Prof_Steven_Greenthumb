...
uint8_t ble_rx_buffer[60];
uint8_t ble_rx_buffer_len = 0;
uint8_t ble_connection_state = false;
uint8_t sendBuffer[]= "Plantasy!";
...
void loop() {
  ...
  if (display.getButtons(TSButtonLowerLeft) && homepage) {
    display.clearScreen();
    settingpage = true;
    homepage = false;
  }
  ...
  if (display.getButtons(TSButtonLowerRight) && settingpage) {
    MainPage();
    settingpage = false;
    homepage = true;
  }
